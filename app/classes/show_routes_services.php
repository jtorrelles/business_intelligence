   ...
   public static function getChangeDataInfo($routeDetailID, $routeID){
      try {
       ...
       if(!$result) {
        ...
        while($resultSet = mysqli_fetch_assoc($result)) {
          $res[$x]["ROUTES_DETID"] = $resultSet['ROUTES_DETID'];
          $res[$x]["PRESENTATION_DATE"] = $resultSet['PRESENTATION_DATE'];
          $x++;       
        }   
      ...
      try {
        ...
        if($imageFileType != "xlsx") {
		  ...
		  ob_clean();
                  ...
                  $DR = gmdate($format = "Y-m-d", PHPExcel_Shared_Date::ExceltoPHP($Date_route));  
                  for($i = 0; $i < 364; $i++){
                    $id = 8 + $i;
                    ...
                    $Repeat[$i] = trim($objPHPExcel->getSheet(1)->getCell("H".$id)->getValue());
                    $Mileage[$i] = trim($objPHPExcel->getSheet(1)->getCell("I".$id)->getValue());
                    ...
                    $Perf[$i] = trim($objPHPExcel->getSheet(1)->getCell("P".$id)->getCalculatedValue());
                    $Venue[$i] = $objPHPExcel->getSheet(1)->getCell("Q".$id)->getValue();
                    $Presenter[$i] = $objPHPExcel->getSheet(1)->getCell("R".$id)->getValue();
                    $Capacity[$i] = trim($objPHPExcel->getSheet(1)->getCell("S".$id)->getCalculatedValue());
                    $Fixed_gntee[$i] = trim($objPHPExcel->getSheet(1)->getCell("T".$id)->getCalculatedValue());
                    $Royalty[$i] = trim($objPHPExcel->getSheet(1)->getCell("U".$id)->getValue());
                    $Backend[$i] = trim($objPHPExcel->getSheet(1)->getCell("V".$id)->getValue());
                    $Breakeven[$i] = trim($objPHPExcel->getSheet(1)->getCell("W".$id)->getValue());
                    $Deal_notes[$i] = $objPHPExcel->getSheet(1)->getCell("X".$id)->getValue();
                    $Est_royalty[$i] = trim($objPHPExcel->getSheet(1)->getCell("Y".$id)->getCalculatedValue());
                    $On_sub[$i] = trim($objPHPExcel->getSheet(1)->getCell("Z".$id)->getValue());
                    $Date_conf[$i] = trim($objPHPExcel->getSheet(1)->getCell("AA".$id)->getValue());
                    $Offer[$i] = trim($objPHPExcel->getSheet(1)->getCell("AB".$id)->getValue());
                    $Price_scales[$i] = trim($objPHPExcel->getSheet(1)->getCell("AC".$id)->getValue());
                    $Expenses[$i] = trim($objPHPExcel->getSheet(1)->getCell("AD".$id)->getValue());
                    $Deal_memo[$i] = trim($objPHPExcel->getSheet(1)->getCell("AE".$id)->getValue());
                    $Contract[$i] = trim($objPHPExcel->getSheet(1)->getCell("AF".$id)->getValue());
                  }   
