...
if(isset($_GET['selectedid'])){
    ...
    $sql1 = "SELECT ro.SHOWID as SHOWID,
                    det.PRESENTATION_DATE as FIRST_DATE
             FROM routes ro, routes_det det 
             WHERE ro.ROUTESID = det.ROUTESID
             AND ro.ROUTESID = $routeid
             ORDER BY det.PRESENTATION_DATE
             LIMIT 1";

    $result = $conn->query($sql1);
    while ($row = $result->fetch_assoc()){
        $showid_orig = $row['SHOWID'];
        $fday_orig = $row['FIRST_DATE'];
    }

    if($showid==$showid_orig){
        if($fday==$fday_orig){
           
            $sql2 = "UPDATE routes 
                     SET DATE_OF_ROUTE = '$date_route'
                     WHERE ROUTESID = $routeid";

            if($conn->query($sql2) === TRUE) {

                $sql3 = "DELETE FROM routes_det WHERE ROUTESID = $routeid";
                ...
                if($conn->query($sql3) === TRUE) {
                    ...
                    for($i = 0; $i < 364; $i++){
                        ...
                        $sql4 = "INSERT INTO routes_det (ROUTESID,PRESENTATION_DATE,HOLIDAY,CITYID,`REPEAT`,MILEAGE,BOOK_NOTES,PROD_NOTES,TIME_ZONE,SHOW_TIMES,PERF,VENUEID,PRESENTERID,CAPACITY,FIXED_GNTEE,ROYALTY,BACKEND,BREAKEVEN,DEAL_NOTES,EST_ROYALTY,ON_SUB,DATE_CONF,OFFER,PRICE_SCALES,EXPENSES,DEAL_MEMO,CONTRACT)
            ...
            echo "The First Date of the Route to be loaded is not the same";
        }      
    }else {
        echo "The Show of the Route to be loaded is not the same";
    }            
}else {
  echo "Error";
}
